import 'package:car_app/bloc/state_provider.dart';
...
class LayoutStars extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return Stack(
      children: <Widget>[
...
class _CarDetailAnimationState extends State<CarDetailAnimation> with TickerProviderStateMixin{

  AnimationController fadeController;
  AnimationController scaleController;

  Animation fadeAnamtion;
  Animation scaleAnamtion;
  ...
  void initState() {
    ...
    fadeAnamtion = Tween(begin: 0.0, end: 1.0).animate(fadeController);
    scaleAnamtion = Tween(begin: 0.8, end: 1.0).animate(CurvedAnimation(
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<Object>(
      ...
      stream: stateBloc.status,
      builder: (context, snapshot) {
        snapshot.data ? forward() : reverse();
        return ScaleTransition(
          scale: scaleAnamtion,
          child: FadeTransition(
            opacity: fadeAnamtion,
            child: CarDetails(),
          ),
        );
      }
  ...
  Widget build(BuildContext context) {
    return Positioned(
      ...
      child: GestureDetector(
        ...
        onVerticalDragEnd: (DragEndDetails dred){
          if(dred.primaryVelocity < 0.0) {
            forwardAnimation();
          }else if(dred.primaryVelocity > 0.0) {
            reverseAnimation();
          }else{
            return;
          }
